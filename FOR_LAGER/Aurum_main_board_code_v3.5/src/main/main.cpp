...
int32_t angleToGo(Camera &cam, Gyroscope &gyro, int32_t angleIR)
{
	double alpha = homeGoalAngle(cam) + gyro.getStartingAngle() - gyro.getChangedAngle() - angleIR + 180;
	double x = GOALDISTANCE * cos(alpha);
	double d = homeGoalDistance(cam);
	double d1 = d * cos(alpha);
	double y = d1 - x;
	double b = d * sin(alpha);
	double lambda = sqrt(y*y + b*b);
	double cosAlpha = (x*x - d*d - lambda*lambda)/(-2*d*lambda);
	int32_t gotoAngle = acos(cosAlpha);
	return gotoAngle;
}
...
//	
	...
	for(int32_t gyroCount = 0; gyroCount < 50000; gyroCount++)
	{
			gyro.setAngle(0);
	}
	dribbler.pwm(1100);
		...
		if(CODE == ATTACKER)
		{
			...
			{	
				...
				{		
					...
					{
						dribbler.pwm(1350);
						if((LB.getValue() || time_service::getCurTime() - throwtimer > 1000) && time_service::getCurTime() - throwtimer > 500)
						{
							throwtimer = time_service::getCurTime();
							startOp = 5;
						}
...
//						}
					...
					else if (startOp == 45)
					{
						...
						{
							int32_t curEnemyGoalAngle = enemyGoalAngle(cam) + gyro.getStartingAngle() - gyro.getChangedAngle();
							math.calculateSpeed(curEnemyGoalAngle, 1000, sp1, sp2, sp3, sp4);
						}
						else if(startOp == 41)
						{
							math.calculateSpeed(180, 750, sp1, sp2, sp3, sp4);
						}
						else if(startOp == 45)
						{
							if(enemyGoalGLY(cam, gyro) > 30)
							{
							int32_t curEnemyGoalAngle = enemyGoalAngle(cam);
							if(curEnemyGoalAngle < 0)
							{
								curEnemyGoalAngle += 360;
							}
							curEnemyGoalAngle += gyro.getStartingAngle() - gyro.getChangedAngle();
							math.calculateSpeed(curEnemyGoalAngle, 1000, sp1, sp2, sp3, sp4);
							}
							else
							{
